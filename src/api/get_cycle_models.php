...
// Get cycle models - if brand specified, filter by brand; otherwise return all models
if(!empty($brand)) {
    $sql = "SELECT title FROM cycle_adds WHERE title LIKE '$brand %' AND title IS NOT NULL ORDER BY title";
} else {
    // Get all models from all brands
    $sql = "SELECT title FROM cycle_adds WHERE title IS NOT NULL ORDER BY title";
}
