...
$allowed_fields = ['description', 'price_per_month', 'security_deposit', 'city', 'latitude', 'longitude', 'frame_material', 'seating_capacity', 'seat_foam_type', 'sofa_type'];
...
foreach($allowed_fields as $field) {
    if(isset($input[$field])) {
        ...
        if($field === 'seating_capacity' && $value <= 0) {
            http_response_code(400);
...
if($conn->query($update_sql)) {
    ...
    $response['message'] = 'Sofa_adds updated successfully';
    $response['data'] = [
        'add_id' => $add_id,
        'table' => $table_name,
