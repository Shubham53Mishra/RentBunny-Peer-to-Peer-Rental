...
// Insert into database using existing table columns (without images - upload separately via image_upload.php)
...
if($conn->query($insert_sql)) {
    ...
    $response['message'] = 'AC ad posted successfully. Upload images using image_upload.php endpoint';
    $response['data'] = [
        'id' => $add_id,
        'user_id' => $user_id,
        'ad_title' => $ad_title,
        ...
        'next_step' => 'Upload images using POST to /api/image_upload.php with add_id=' . $add_id
    ];
    http_response_code(200);
} else {
    http_response_code(500);
    $response['success'] = false;
    $response['message'] = 'Failed to post AC ad';
