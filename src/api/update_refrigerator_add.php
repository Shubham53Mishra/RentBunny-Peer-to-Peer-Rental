...
$height = isset($input['height']) ? mysqli_real_escape_string($conn, $input['height']) : null;
$length = isset($input['length']) ? mysqli_real_escape_string($conn, $input['length']) : null;
$width = isset($input['width']) ? mysqli_real_escape_string($conn, $input['width']) : null;
$capacity = isset($input['capacity']) ? mysqli_real_escape_string($conn, $input['capacity']) : null;
$defrosting_type = isset($input['defrosting_type']) ? mysqli_real_escape_string($conn, $input['defrosting_type']) : null;
...
if($conn->query($update_sql)) {
    ...
    $response['message'] = 'Refrigerator ad updated successfully';
    $response['data'] = [
        'add_id' => $add_id,
    ...
    $response['message'] = 'Failed to update refrigerator ad: ' . $conn->error;
}
