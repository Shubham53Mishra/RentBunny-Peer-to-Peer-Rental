...
$required_fields = ['processor', 'model', 'ram', 'graphic_card', 'os', 'ssd', 'hdd', 'ram_type', 'brand', 'product_type', 'price_per_month', 'security_deposit', 'ad_title', 'description', 'latitude', 'longitude', 'city', 'image_urls'];
...
$processor = mysqli_real_escape_string($conn, $input['processor']);
$model = mysqli_real_escape_string($conn, $input['model']);
$ram = mysqli_real_escape_string($conn, $input['ram']);
$graphic_card = mysqli_real_escape_string($conn, $input['graphic_card']);
$os = mysqli_real_escape_string($conn, $input['os']);
$ssd = mysqli_real_escape_string($conn, $input['ssd']);
$hdd = mysqli_real_escape_string($conn, $input['hdd']);
$ram_type = mysqli_real_escape_string($conn, $input['ram_type']);
...
if($conn->query($update_sql)) {
    ...
    $fetch_sql = "SELECT id, user_id, title, description, price, city, latitude, longitude, image_url, brand FROM $table_name WHERE id = '$add_id'";
    $fetch_result = $conn->query($fetch_sql);
    $updated_record = $fetch_result ? $fetch_result->fetch_assoc() : null;
    
    $response['success'] = true;
    $response['message'] = 'Computer ad updated successfully';
    $response['data'] = [
        'id' => $add_id,
        'user_id' => $user_id,
    ...
    $response['message'] = 'Failed to update computer ad';
