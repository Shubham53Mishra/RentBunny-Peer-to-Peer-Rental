...
$processor = isset($input['processor']) ? mysqli_real_escape_string($conn, $input['processor']) : null;
$model = isset($input['model']) ? mysqli_real_escape_string($conn, $input['model']) : null;
$ram = isset($input['ram']) ? mysqli_real_escape_string($conn, $input['ram']) : null;
$graphic_card = isset($input['graphic_card']) ? mysqli_real_escape_string($conn, $input['graphic_card']) : null;
$os = isset($input['os']) ? mysqli_real_escape_string($conn, $input['os']) : null;
$ssd = isset($input['ssd']) ? mysqli_real_escape_string($conn, $input['ssd']) : null;
$hdd = isset($input['hdd']) ? mysqli_real_escape_string($conn, $input['hdd']) : null;
$ram_type = isset($input['ram_type']) ? mysqli_real_escape_string($conn, $input['ram_type']) : null;
...
if($conn->query($update_sql)) {
    ...
    $response['message'] = 'Laptop ad updated successfully';
    $response['data'] = [
        'add_id' => $add_id,
    ...
    $response['message'] = 'Failed to update laptop ad: ' . $conn->error;
}
